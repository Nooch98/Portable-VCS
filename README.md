...
#### Dashboard Preview (Intelligent IDE Diff-View)
The new comparison system leverages an embedded **Monaco Editor** interface, providing a high-fidelity split-view diff. It automatically aligns code lines, offers syntax-aware highlighting, and provides a navigable minimap, making the review of complex snapshots effortless.

<img width="1825" height="999" alt="Captura de pantalla 2026-06-05 181320" src="https://github.com/user-attachments/assets/74899896-1678-49e9-b18b-54cfefeb842b" />
...
Portable VCS includes an advanced automation engine that acts as a **Local Continuous Integration (CI)** system. By running custom scripts before every `push`, you ensure your snapshots meet project standards before they are committed to the vault.

### New: Intelligent Hook Editing
I replaced manual text editing with an **Embedded Monaco IDE**. When you run `vcs hook edit <name>`, the system spawns a dedicated, localized web-IDE that provides:
* **Syntax Highlighting**: Automatic detection for PowerShell, Bash, and Shell.
* **Integrated Testing Terminal**: Execute and debug your hooks live within the IDE before saving.
* **Minimalist UI**: Spawns in a clean, focused window for distraction-free development.
...
| `vcs hook edit <name>` | **Opens the professional IDE interface** for your hook. |
| `vcs hook exec <name>` | Manually triggers a hook with full context injection. |
| `vcs hook clean` | Purges all execution logs generated by hooks. |
| `vcs hook delete <name>` | Removes the hook configuration and its script file. |

### Example: Validation Hook
1. **Create:** `vcs hook create validate --config auto`
2. **Edit:** Run `vcs hook edit validate`. The IDE will open. Paste your logic:
   ```bash
   ...
   ```
3. **Test:** Click 🧪 Run Test inside the IDE to verify it works.
4. **Save:** Click 💾 Save Changes. Now, your `vcs push` will automatically abort if the analysis fails.

### Hook Observability
Every hook execution is logged. If a hook fails, you can inspect the high-fidelity logs (STDOUT/STDERR) using:
```bash
vcs hook log <name>
```
or
...
| | `vcs ancestry --track/-t <name>` | Visualize ancestry of a specific track. |
